...
def load_inference_function(
    model_path: str, device: torch.device | None = None, max_tile_size: int = 1024
) -> tuple[Callable[[torch.Tensor], torch.Tensor], list[int]]:
    """
...
def main(
    ...
    inference_function, channels = load_inference_function(
        experiment_path, device, max_tile_size=max_tile_size
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max_tile_size", help="Size to tile the GeoTIFFs", type=int, default=1_024
    )
    parser.add_argument("--path_to_splits", required=True, help="path to test and val folders")
    parser.add_argument(
        "--train_test_split_file", default="", help="split file with test and validation tiffs"
    )
    ...
    if args.experiment_path == "":
        ...
        if args.experiment_folder.startswith("gs"):
            ...
            prefix = ""
        experiment_paths = [
            f"{prefix}{os.path.dirname(f)}"
            for f in fs.glob(
                os.path.join(args.experiment_folder, "*", "config.json").replace("\\", "/")
            )
        ]
        assert len(experiment_paths) > 0, "No models found in " + os.path.join(
            args.experiment_folder, "*", "config.json"
        ).replace("\\", "/")
    else:
