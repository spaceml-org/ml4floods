...
def load_input(
    tiff_input: str, channels: list[int], window: rasterio.windows.Window | None = None
...
def main(cems_code: str, aoi_code: str):
    tiff_files = fs.glob(
        f"gs://ml4cc_data_lake/0_DEV/1_Staging/WorldFloods/*{cems_code}/*{aoi_code}/S2/*.tif"
    )
    ...
    for total, filename in enumerate(tiff_files):
        ...
        filename_save_vect = filename.replace("/S2/", "/cmkappazeta_vec/").replace(
            ".tif", ".geojson"
        )
        ...
        print(
            f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ({total}/{len(tiff_files)}) Processing {filename}"
        )
        ...
        try:
            if exists_tiff:
                ...
                inputs, transform, crs = load_input(
                    filename, channels=[0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12]
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Run inference and vectorize the output of kappazeta cloud and cloud shadow mask"
    )
    parser.add_argument("--cems_code", default="", help="EMS Codes to filter")
    parser.add_argument(
        "--aoi_code",
        default="",
