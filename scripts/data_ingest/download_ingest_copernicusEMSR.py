...
PATH_TO_WRITE_UNZIP = (
    "gs://ml4cc_data_lake/0_DEV/0_Raw/WorldFloods/copernicus_ems/copernicus_ems_unzip"
)
...
def main():
    ...
    output_temp_folder = (
        output_temp_folder[:-1] if output_temp_folder.endswith("/") else output_temp_folder
    )
    output_temp_folder_unzip = f"{output_temp_folder}_raw"

    # retrieve zipfile urls for each activation EMSR code
    print("Generating Copernicus EMSR codes to fetch:")
    ...
    for _i, emsr_code in enumerate(emsr_codes):
        ...
        for _j, zip_url in enumerate(zip_files_activation_url_list):
            ...
            print(
                f"\t{_j + 1}/{len(zip_files_activation_url_list)} Processing Code {emsr_code} AoI {aoi_code} file {name_zip}"
            )

            path_to_write_unzip_bucket = (
                f"{PATH_TO_WRITE_UNZIP}/{emsr_code}/{aoi_code}/{product_name}/"
            )
            path_to_write_zip_bucket = f"{PATH_TO_WRITE_ZIP}/{emsr_code}/{aoi_code}/{name_zip}"

            gcp_metadata_floodmap_dir = os.path.join(
                PATH_TO_WRITE_PROCESSED_DATA, emsr_code, aoi_code, "flood_meta"
            )

            gcp_floodmap_dir = os.path.join(
                PATH_TO_WRITE_PROCESSED_DATA, emsr_code, aoi_code, "floodmap"
            )
            ...
            if not fs_bucket.exists(path_to_write_zip_bucket) or (len(filesunzip) == 0):
                ...
                unzipfullpath = activations.unzip_copernicus_ems(
                    zipfullpath, output_temp_folder_unzip
                )
                ...
                print(
                    f"\tFile {path_to_write_zip_bucket} and {path_to_write_unzip_bucket} exists. Obtaining from bucket"
                )
            ...
            try:
                metadata_floodmap = activations.filter_register_copernicusems(
                    unzipfullpath, code_date, verbose=True
                )
                ...
                gcp_metadata_floodmap_path = os.path.join(
                    gcp_metadata_floodmap_dir, satellite_date.strftime("%Y-%m-%d") + ".pickle"
                )

                gcp_floodmap_path = os.path.join(
                    gcp_floodmap_dir, satellite_date.strftime("%Y-%m-%d") + ".geojson"
                )

                if fs_bucket.exists(gcp_metadata_floodmap_path) and fs_bucket.exists(
                    gcp_floodmap_path
                ):
                    print(
                ...
                utils.write_pickle_to_gcp(
                    gs_path=gcp_metadata_floodmap_path, dict_val=metadata_floodmap
                )
