...
def main(
    ...
    path_to_glob = os.path.join(
        metadatas_path, f"{cems_code}*", f"{aoi_code}*", "flood_meta", "*.pickle"
    ).replace("\\", "/")
    ...
    for _i, meta_floodmap_filepath in enumerate(files_metatada_pickled):
        try:
            ...
            satellite_date = datetime.strptime(
                metadata_floodmap["satellite date"].strftime("%Y-%m-%d %H:%M:%S"),
                "%Y-%m-%d %H:%M:%S",
            ).replace(tzinfo=UTC)
            ...
            date_end_search = min(
                datetime.today().astimezone(UTC), satellite_date + timedelta(days=days_after)
            )

            print(
                f"{_i + 1}/{len(files_metatada_pickled)} processing {meta_floodmap_filepath} between {date_start_search.strftime('%Y-%m-%d')} and {date_end_search.strftime('%Y-%m-%d')}"
            )
            ...
            def filter_images(img_col_info_local: pd.DataFrame) -> pd.Series:
                is_image_same_solar_day = img_col_info_local["datetime"].apply(
                    lambda x: (satellite_date - x).total_seconds() / 3600.0 < 10
                )
                filter_before = (
                    (img_col_info_local["cloud_probability"] <= threshold_clouds_before)
                ...
                filter_after = (
                    (img_col_info_local["cloud_probability"] <= threshold_clouds_after)
            ...
            for collection_name_trigger, resolution_meters in zip(
                collection_names, resolutions_meters, strict=False
            ):
                folder_dest_s2 = os.path.join(folder_dest, collection_name_trigger)
                name_task = collection_name_trigger + "_" + basename_task
                tasks_iter.extend(
                    ee_download.download_s2l89(
                        pol_scene_id,
            ...
            if len(tasks_iter) > 0:
                ...
                print("\tAll S2 data downloaded for product")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "Download Sentinel-2 and Landsat-8/9 images for floodmaps in Staging"
    )
    parser.add_argument(
        "--cems_code",
        default="",
    ...
    parser.add_argument(
        "--metadatas_path",
        default="gs://ml4cc_data_lake/0_DEV/1_Staging/WorldFloods/",
    ...
    parser.add_argument(
        "--days_before", default=20, type=int, help="Days to search after the event"
    )
    parser.add_argument(
        "--margin_pre_search",
        default=0,
    ...
    parser.add_argument(
        "--days_after", default=20, type=int, help="Days to search before the event"
    )
    ...
    main(
        ...
        metadatas_path=args.metadatas_path,
