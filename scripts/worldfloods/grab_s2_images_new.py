...
from src.data.utils import remove_gcp_prefix, get_files_in_directory_gcp, read_pickle_from_gcp
from typing import Tuple
from collections import namedtuple
import tqdm


ActivationFile = namedtuple("ActivationFile", ["directory_aoi", "event_activation", "file_name"])
...
def find_leaf_nodes(filepath: str) -> Tuple[str, str]:
    ...
    return ActivationFile(directory_aoi=directory_aoi, event_activation=event_aoi, file_name=file_name)
...
#                     pbar_files.set_description(f"AOI: {i_file}")
                    ...
                    # Load Floodmap geojson
                    meta_floodmap_filepath = "gs://" + str(Path(i_file.replace("/floodmap/", "/flood_meta/").replace(".geojson", ".pickle").replace("_floodmap.", "_metadata_floodmap.")))
                    ...
                    filename = Path(PARENT_DIR_S2).joinpath(activation_aoi_meta.event_activation).joinpath(activation_aoi_meta.directory_aoi).joinpath(activation_aoi_meta.file_name.replace("_floodmap.geojson", ""))
