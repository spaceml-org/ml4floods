...
def main(
    ...
    for _i, row in enumerate(aois_data.itertuples()):
        try:
            ...
            date_end_search = min(
                datetime.today().astimezone(UTC), flood_date + timedelta(days=days_after)
            )

            print(
                f"{_i + 1}/{aois_data.shape[0]} processing images between {date_start_search.strftime('%Y-%m-%d')} and {date_end_search.strftime('%Y-%m-%d')}"
            )
            ...
            def filter_images(img_col_info_local: pd.DataFrame) -> pd.Series:
                is_image_same_solar_day = img_col_info_local["datetime"].apply(
                    lambda x: (flood_date - x).total_seconds() / 3600.0 < 10
                )
                filter_before = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--flood_date", required=True, help="Reference date to download images (YYYY-mm-dd)"
    )
    parser.add_argument(
        "--aoi_code",
        default="",
    ...
    parser.add_argument(
        "--bucket_path",
        default="gs://ml4cc_data_lake/0_DEV/1_Staging/operational/",
        help="Path to store inference activations in bucket. Default %(default)s",
    )
    parser.add_argument(
        "--collection_name",
        choices=["Landsat", "S2", "both"],
        default="both",
    ...
    flood_date = datetime.strptime(args.flood_date, "%Y-%m-%d").replace(tzinfo=UTC)
    main(
        args.cems_code,
