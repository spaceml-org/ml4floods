...
def load_inf_function_as_v1(config, model):
    if config["model_params"].get("model_version", "v1") == "v2":
        inf_func = get_model_inference_function(
...
def compute_metrics_v1(
    ...
    if not overwrite and all(
        fs.exists(
            os.path.join(experiment_path, f"{experiment_name}/{dl_name}.json").replace("\\", "/")
        )
        for dl_name in ["test", "val"]
    ):
    ...
    config.data_params.train_test_split_file = (
        "2_PROD/2_Mart/worldfloods_v1_0/train_test_split.json"
    )
    ...
    for dl, dl_name in [
        ...
        mets = metrics.compute_metrics(
            dl, inference_function, thresholds_water=thresholds_water, plot=False
        )
        ...
        print("Per Class IOU", json.dumps(mets["iou"], indent=4, sort_keys=True))
...
if __name__ == "__main__":
    ...
    for e in tqdm(experiments_dev):
        try:
            compute_metrics_v1(
                e, experiment_path=experiment_path, path_to_splits=path_2_splits, device=device
            )
        except Exception:
