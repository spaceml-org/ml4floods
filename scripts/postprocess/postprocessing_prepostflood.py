...
def main(
    ...
    flooding_date_pre: str,
    flooding_date_post_start: str,
    flooding_date_post_end: str,
    ...
    geojsons = sorted(
        [
            f"{prefix}{f}"
            for f in fs.glob(os.path.join(model_output_folder, "*", "*.geojson").replace("\\", "/"))
        ]
    )
    ...
    for _iaoi, aoi in enumerate(aois):
        ...
        pre_flood_path = os.path.join(
            aoi_folder, "pre_post_products", f"preflood_{flooding_date_pre}.geojson"
        ).replace("\\", "/")
        ...
        post_flood_path = os.path.join(
            aoi_folder,
            "pre_post_products",
            f"postflood_{flooding_date_post_start}_{flooding_date_post_end}.geojson",
        ).replace("\\", "/")
        ...
        geojsons_post = [
            ...
            if (os.path.splitext(os.path.basename(g))[0] >= flooding_date_post_start)
            and (os.path.splitext(os.path.basename(g))[0] <= flooding_date_post_end)
        ]
        ...
        try:
            ...
            if (not overwrite) and fs.exists(post_flood_path):
                ...
                if permanent_water_floodmap is not None:
                    best_post_flood_data = postprocess.add_permanent_water_to_floodmap(
                        permanent_water_floodmap, best_post_flood_data, water_class="water"
                    )
            ...
            if (not overwrite) and fs.exists(pre_flood_path):
                ...
                if len(geojsons_pre) != 0:
                    ...
                    best_pre_flood_data = gpd.GeoDataFrame({"geometry": [], "class": []}).set_crs(
                        best_post_flood_data.crs
                    )
                    print(f"\tNo pre-flood image found for aoi: {aoi}. Trying with permanent water")
                # Add permanent water polygons
                if permanent_water_floodmap is not None:
                    print("\tAdding permanent water for pre flood data")
                    best_pre_flood_data = postprocess.add_permanent_water_to_floodmap(
                        permanent_water_floodmap, best_pre_flood_data, water_class="water"
                    )
            ...
            for geojson_post in geojsons_post:
                ...
                floodmap_post_data_pre_post = postprocess.compute_pre_post_flood_water(
            ...
            if overwrite or not fs.exists(prepost_flood_path):
                prepost_flood_data = postprocess.compute_pre_post_flood_water(
                    best_post_flood_data, best_pre_flood_data
                )
    ...
    path_aggregated_post = os.path.join(
        activation_folder, f"postflood_{flooding_date_post_start}_{flooding_date_post_end}.geojson"
    )
    path_aggregated_prepost = os.path.join(
        activation_folder,
        f"prepostflood_{flooding_date_pre}_{flooding_date_post_start}_{flooding_date_post_end}.geojson",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_output_folder",
        required=True,
        help="Path to model output folder to run it for all AoIs:"
        "gs://ml4cc_data_lake/0_DEV/1_Staging/operational/EMSR570/*/WF2_unet_rbgiswirs_vec"
