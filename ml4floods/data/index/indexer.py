...
def index_worldfloods(fs_prefix: str, out_path: str) -> None:
    ...
    print(
        "Note: This is practically instant with local block storage, but may take several minutes if running from a cloud storage bucket (GCP, AWS, Azure, etc.)"
    )
    with open_fs(fs_prefix + os.path.join(MASTER_PREFIX, METADATA_PREFIX) + "?strict=False") as fs:
        for file in fs.listdir(""):
            if file.endswith(".json"):
                ...
                with open_fs(
                    fs_prefix
                    + os.path.join(MASTER_PREFIX, METADATA_PREFIX, file)
                    + "?strict=False",
                    "r",
                ) as fs2:
                ...
                if "bounds" in metadata:
                    ...
                    if file_exists(
                        os.path.join(fs_prefix, MASTER_PREFIX, FLOODMAP_PREFIX, basename + ".shp")
                    ):
                        relevant_files.append(
                            {
                                ...
                                "path": os.path.join(
                                    MASTER_PREFIX, FLOODMAP_PREFIX, basename + ".shp"
                                ),
                            }
                        )

                    if file_exists(
                        os.path.join(fs_prefix, MASTER_PREFIX, CLOUDMASK_PREFIX, basename + ".tif")
                    ):
                        relevant_files.append(
                            {
                                "type": "cloudmask",
                                "last_modified": satdate,
                                "path": os.path.join(
                                    MASTER_PREFIX, CLOUDMASK_PREFIX, basename + ".tif"
                                ),
                            }
                        )
                    ...
                    for sat_prefix in PROVIDER_GEOTIFF_PREFIXES:
                        sat_path = os.path.join(
                            fs_prefix, MASTER_PREFIX, sat_prefix, basename + ".tif"
                        )
                        if file_exists(sat_path):
                            relevant_files.append(
                                {
                                    "type": "satellite_image",
                                    "last_modified": s2satdate if sat_prefix == "S2" else satdate,
                                    "provider_id": sat_prefix,
                                    "path": os.path.join(
                                        MASTER_PREFIX, sat_prefix, basename + ".tif"
                                    ),
                                }
                            )
                    ...
                    relevant_files.append(
                        {
                            ...
                            "path": os.path.join(
                                MASTER_PREFIX, METADATA_PREFIX, basename + ".json"
                            ),
                        }
                    )
