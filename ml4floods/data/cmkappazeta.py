...
def vectorize_output(
    prediction: np.ndarray, crs: str, transform: rasterio.transform.Affine
    ...
    for c in [2, 3, 4]:
        ...
        geoms_polygons = vectorize.get_polygons(binary_mask, transform=transform)
...
def predbytiles(
    pred_function: Callable, input_batch: np.ndarray, tile_size=1280, pad_size=32
) -> np.ndarray:
    """
    ...
    for i, j in itertools.product(
        range(0, input_batch.shape[0], tile_size), range(0, input_batch.shape[1], tile_size)
    ):
        slice_current = (
            slice(i, min(i + tile_size, input_batch.shape[0])),
            slice(j, min(j + tile_size, input_batch.shape[1])),
        )
        slice_pad = (
            slice(max(i - pad_size, 0), min(i + tile_size + pad_size, input_batch.shape[0])),
        ...
        assert cnn_out.ndim == 3, (
            f"Expected 3-band prediction (after softmax) found {cnn_out.shape}"
        )

        if pred_continuous_tf is None:
            pred_continuous_tf = np.zeros(
                (input_batch.shape[0], input_batch.shape[1], cnn_out.shape[-1]), dtype=np.float32
            )
...
class Unet:
    ...
    def __init__(self, version: str = "L1C"):
        ...
        self.max_v = np.array(
            [0.19, 0.298, 0.279, 0.293, 0.243, 0.247, 0.317, 0.354, 0.153, 0.055, 0.24, 0.23]
        )
    ...
    def load_weights(
        self,
        path="gs://ml4cc_data_lake/0_DEV/2_Mart/2_MLModelMart/kappazeta/l1c__091-0.42.hdf5",
        filelocal=None,
    ):
        if path.startswith("gs"):
            ...
            if filelocal is None:
                with tempfile.NamedTemporaryFile(
                    dir=".", suffix=".hdf5", prefix=os.path.basename(path), delete=True
                ) as fileobj:
    ...
    def predict(self, input: np.ndarray, tile_size: int = 1280) -> np.ndarray:
        ...
        assert input.shape[0] == len(self.features), (
            f"Input tensor shape {input.shape} unexpected\n"
        ...
        if (data_to_pred_tf.shape[0] > tile_size) or (data_to_pred_tf.shape[1] > tile_size):
            classes_prob = predbytiles(
                lambda patch: self.model.predict(patch[None])[0], data_to_pred_tf
            )
        else:
