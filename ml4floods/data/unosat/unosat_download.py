...
def _page_links(response):
    """Return the set of hrefs on a page."""
    soup = BeautifulSoup(response.text, "html.parser")
    return {a["href"] for a in soup.find_all("a", href=True)}


def get_map_links(fetched_page):
    links = _page_links(fetched_page)
    map_links = list(filter(lambda x: re.match(r"^/maps/map/[0-9]+", x), links))

    next_page_links = list(filter(lambda x: re.match(r"\?page=[0-9]+", x), links))
...
def get_flood_shape_link(fetched_page, download_base):
    glide_ids = re.findall(
        r"GLIDE\:( |&nbsp;)(FL[0-9]+[A-Z]+)", fetched_page.content.decode(fetched_page.encoding)
    )
    if len(glide_ids) > 0:
        ...
        shape_links = list(
            filter(
                lambda x: re.match(
                    f"{download_base}[a-zA-Z0-9\\-/]+{glide_ids[0][1]}_(SHP|shp)\\.zip", x
                ),
                _page_links(fetched_page),
            )
        )
...
def get_country_links_from_url(session, country_list_url):
    ...
    return list(
        filter(lambda x: re.match(r"^/maps/countries/[0-9]+", x), _page_links(fetched_page))
    )
...
def get_flood_shape_published_date(fetched_page):
    published_dates = re.findall(
        r"Published\:( |&nbsp;)([0-9A-Za-z ,]+)", fetched_page.content.decode(fetched_page.encoding)
    )
...
def get_satellite_source(fetched_page):
    matches = re.findall(
        r"Satellite Data( |&nbsp;)*(\((Post|Pre|[0-9]+)\))?( |&nbsp;)*\:( |&nbsp;)*([0-9A-Za-z ,\-]+)",
        fetched_page.content.decode(fetched_page.encoding),
    )
    if len(matches) > 0:
        return matches[0][5]
    return None
...
class ShapeFileInfo:
    ...
    def __hash__(self):
        return (
            self.download_url.__hash__() + self.date_published.__hash__() + self.glide_id.__hash__()
        )
...
def produce_metadata_dict(shapefile_info, shapefile_path):
    ...
    meta = {
        ...
        "satellite date": shapefile_info.imagery_dates
        if shapefile_info.imagery_dates is not None
        else "NaN",
        "country": "NaN",
        "satellite": shapefile_info.satellite_data
        if shapefile_info.satellite_data is not None
        else "NaN",
...
def get_shapefile_from_zip(zip_file):
    ...
    with ZipFile(zip_file, "r") as f:
        ...
        flood_extent_shapefiles = list(
            filter(lambda x: re.match(r".*\_FloodExtent\_.*\.shp$", x), contents)
        )
        if len(flood_extent_shapefiles) > 0:
            if len(flood_extent_shapefiles) != 1:
                print(
                    "WARN: More than one flood shapefile... We will only be using the first one..."
                )
            ...
            for relevant_file in relevant_shapefiles:
                try:
                    with (
                        f.open(relevant_file, "r") as shp_f,
                        open(f"./tmp/{relevant_file}", "wb") as f_out,
                    ):
...
def run_unosat_pipeline(
    base_url, country_list_url, download_base_regex, shapefile_bucket, metadata_bucket
):
    session = requests.Session()
    flood_shapefiles = get_flood_shapefiles(
        session, base_url, country_list_url, download_base_regex
    )
