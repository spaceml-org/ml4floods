...
def _filter_windows(
    fun_frac_clouds_invalids: Callable,
    dataset: WorldFloodsDatasetTiled,
...
def filter_windows_v1(
    dataset: WorldFloodsDatasetTiled, threshold_clouds: float = 0.5
) -> list[WindowSlices]:
    """filter windows from the dataset with more that threshold_clouds * 100 of clouds or invalids"""

    return _filter_windows(
        lambda label: ((label == 0) | (label == 3)).sum() / np.prod(label.shape),
        dataset,
...
def filter_windows_v2(
    ...
    return _filter_windows(
        lambda label: (label[1] == 0).sum() / np.prod(label.shape[1:]),
        dataset,
        threshold_clouds=threshold_clouds,
    )
