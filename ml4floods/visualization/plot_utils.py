...
INTERPRETATION_WORLDFLOODS_PERMANENT = [
    "invalid",
    "land",
    "flood water",
    "cloud",
...
def get_cmap_norm_colors(color_array, interpretation_array):
    ...
    norm_categorical = colors.Normalize(vmin=-0.5, vmax=color_array.shape[0] - 0.5)
    patches = []
    for c, interp in zip(color_array, interpretation_array, strict=False):
...
def _read_data(
    filename: str,
    bands_rasterio: list[int] | None = None,
    window: rasterio.windows.Window | None = None,
    size_read: int | None = None,
    ...
    if input_output_factor is not None:
        transform = rasterio.Affine(
            transform.a * input_output_factor,
            transform.b,
            transform.c,
            transform.d,
            transform.e * input_output_factor,
            transform.f,
        )
...
def show(add_scalebar: bool = False, kwargs_scalebar: dict[str, Any] | None = None, **kwargs):
    """
    ...
    if add_scalebar:
        try:
            ...
            raise ImportError(
                f"Install matplotlib-scalebar to use scalebarpip install matplotlib-scalebar{e}"
            )
...
def plot_rgb_image(
    ...
    transform: rasterio.Affine | None = None,
    window: rasterio.windows.Window | None = None,
    max_clip_val: float | None = 3000.0,
    min_clip_val: float | None = 0.0,
    ...
    **kwargs,
    ...
    assert collection_name in {"Landsat", "S2"}, (
        f"Collection name {collection_name} not implemented"
    )
    if not isinstance(input, str):
        ibands = get_channel_configuration_bands(
            channel_configuration, collection_name=collection_name
        )
    ...
    image, transform = get_image_transform(
        input, transform=transform, bands=bands, window=window, size_read=size_read
    )
...
def plot_floodmap(
    ...
    polygons_plot_boundary = (floodmap["class"] == "area_imaged") | (
        floodmap["class"] == "area_imaged-pre-flood"
    )

    if not polygons_plot_boundary.all():
        ax = floodmap[~polygons_plot_boundary].plot(
            color=floodmap_color[~polygons_plot_boundary], figsize=figsize, ax=ax
        )

    # Plot area imaged
    if polygons_plot_boundary.any():
        ax = floodmap[polygons_plot_boundary].boundary.plot(
            color=floodmap_color[polygons_plot_boundary], ax=ax
        )

    if legend:
        legend_elements = [
            Patch(facecolor=COLORS_FLOODMAP[c], label=c) for c in floodmap["class"].unique()
        ]
        ax.legend(handles=legend_elements)
    return ax


def plot_swirnirred_image(
    input: str | np.ndarray,
    transform: rasterio.Affine | None = None,
    window: rasterio.windows.Window | None = None,
    max_clip_val: float | None = 3000.0,
...
def plots_preds_v1(
    ...
    **kwargs,
    ...
    """
    prediction, transform = get_image_transform(
        prediction, transform=transform, bands=[0], window=window, size_read=size_read
    )
    prediction_show = prediction[0] + 1
    cmap_preds, norm_preds, patches_preds = get_cmap_norm_colors(
        configs.COLORS_WORLDFLOODS, INTERPRETATION_WORLDFLOODS
    )
    ...
    if legend:
        ax.legend(handles=patches_preds, loc="upper right")
...
def plots_preds_v2(
    ...
    **kwargs,
    ...
    cmap_preds, norm_preds, patches_preds = get_cmap_norm_colors(
        configs.COLORS_WORLDFLOODS_INVLANDWATER, INTERPRETATION_INVLANDWATER
    )

    ax = show(
        source=prediction_show,
        transform=transform,
        cmap=cmap_preds,
        norm=norm_preds,
        interpolation="nearest",
...
def plot_gt_v1(
    ...
    **kwargs,
    ...
    target, transform = get_image_transform(
        target, transform=transform, bands=[0], window=window, size_read=size_read
    )
    target = target[0]
    cmap_preds, norm_preds, patches_preds = get_cmap_norm_colors(
        configs.COLORS_WORLDFLOODS, INTERPRETATION_WORLDFLOODS
...
def plot_gt_v2(
    ...
    **kwargs,
    ...
    target, transform = get_image_transform(
        target, transform=transform, bands=[0, 1], window=window, size_read=size_read
    )
...
def plot_gt_v1_with_permanent(
    target: str | np.ndarray,
    permanent: str | np.ndarray | None = None,
    transform: rasterio.Affine | None = None,
    window: rasterio.windows.Window | None = None,
    legend=True,
    size_read: int | None = None,
    **kwargs,
    ...
    bands = [0]
    target, transform = get_image_transform(
        target, transform=transform, bands=bands, window=window, size_read=size_read
    )
    target = target[0]
    if permanent is not None:
        permanent, _ = get_image_transform(
            permanent, transform=transform, bands=bands, window=window, size_read=min(target.shape)
        )
        permanent = permanent[0]
        target = gt_v1_with_permanent_water(target, permanent)

    cmap_gt, norm_gt, patches_gt = get_cmap_norm_colors(
        COLORS_WORLDFLOODS_PERMANENT, INTERPRETATION_WORLDFLOODS_PERMANENT
    )
    ...
    if legend:
        ax.legend(handles=patches_gt, loc="upper right")

    return ax


def plot_s2_and_confusions(
    input: str | np.ndarray,
    ...
    **kwargs,
    ...
    band_names_current_image = [
        BANDS_S2[iband] for iband in get_channel_configuration_bands(channel_configuration)
    ]
    bands = [band_names_current_image.index(b) for b in ["B11", "B8", "B4"]]
    ...
    return show(
        source=np.moveaxis(image, -1, 0), cmap=cmap, transform=transform, title=title, **kwargs
    )
...
def plot_segmentation_mask(
    ...
    color_array,
    transform: rasterio.Affine | None = None,
    interpretation_array: list[str] | None = None,
    legend: bool = True,
    ax: matplotlib.axes.Axes | None = None,
) -> matplotlib.axes.Axes:
    """
    ...
    if interpretation_array is not None:
        assert len(interpretation_array) == color_array.shape[0], (
            f"Different numbers of colors and interpretation {len(interpretation_array)} {color_array.shape[0]}"
        )
...
def plot_cloudSEN12mask(mask, legend: bool = True, ax=None):
    ...
    return plot_segmentation_mask(
        mask=mask,
        color_array=COLORS_CLOUDSEN12,
        interpretation_array=INTERPRETATION_CLOUDSEN12,
        legend=legend,
        ax=ax,
    )
