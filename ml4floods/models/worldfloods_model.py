...
from ml4floods.data.worldfloods.configs import (
    ...
    COLORS_WORLDFLOODS,
    COLORS_WORLDFLOODS_INVCLEARCLOUD,
    COLORS_WORLDFLOODS_INVLANDWATER,
)
from ml4floods.models.architectures.baselines import SimpleCNN, SimpleLinear
...
class WorldFloodsModel(pl.LightningModule):
    ...
    def __init__(self, model_params: AttrDict, normalized_data: bool = True):
        ...
        self.weight_per_class = torch.Tensor(
            h_params_dict.get("weight_per_class", [1 for i in range(self.num_class)]),
            device=self.device,
        )
    ...
    def training_step(self, batch: dict, batch_idx) -> float:
        ...
        loss = losses.calc_loss_mask_invalid(
            logits, y, weight=self.weight_per_class.to(self.device)
        )
        if (batch_idx % 100) == 0:
            self.log("loss", loss)
    ...
    def image_to_logger(self, x: torch.Tensor) -> np.ndarray | None:
        return batch_to_unnorm_rgb(
            x,
    ...
    def log_images(self, x, y, logits, prefix=""):
        ...
        if img_data is not None:
            self.logger.experiment.log(
                {
                    f"{prefix}overlay": [
                        self.wb_mask(img, pred, mask)
                        for (img, pred, mask) in zip(img_data, pred_data, mask_data, strict=False)
                    ]
                }
            )
        ...
        self.logger.experiment.log(
            {f"{prefix}y": [wandb.Image(mask_to_rgb(img)) for img in mask_data]}
        )
        self.logger.experiment.log(
            {f"{prefix}pred": [wandb.Image(mask_to_rgb(img + 1)) for img in pred_data]}
        )
    ...
    def validation_step(self, batch: dict, batch_idx):
        ...
        bce_loss = losses.cross_entropy_loss_mask_invalid(
            logits, y, weight=self.weight_per_class.to(self.device)
        )
        ...
        cm_batch = metrics.compute_confusions(
            y, pred_categorical, num_class=self.num_class, remove_class_zero=True
        )
    ...
    def labels(self):
        return {0: "invalid", 1: "land", 2: "water", 3: "cloud"}
...
class ML4FloodsModel(pl.LightningModule):
    ...
    def __init__(self, model_params: AttrDict, normalized_data: bool = True):
        ...
        self.pos_weight = h_params_dict.get("pos_weight", [1 for i in range(self.num_class)])
    ...
    def training_step(self, batch: dict, batch_idx) -> float:
        ...
        logits = self.network(x)
        loss = losses.calc_loss_multioutput_logistic_mask_invalid(
            logits, y, pos_weight_problem=self.pos_weight, weight_problem=self.weight_problem
        )
    ...
    def log_images(self, x, y, logits, prefix=""):
        ...
        for i in range(self.num_class):
            ...
            self.logger.experiment.log(
                {
                    f"{prefix}{problem_name}_pred_cont": [
                        wandb.Image(img[i], mode="L") for img in pred_data
                    ]
                }
            )
            self.logger.experiment.log(
                {
                    f"{prefix}{problem_name}_pred": [
                        wandb.Image(
                            mask_to_rgb(
                                img[i].round().astype(np.int64) + 1,
                                values=[0, 1, 2],
                                colors_cmap=self.colormaps[i],
                            )
                        )
            ...
            self.logger.experiment.log(
                {
                    f"{prefix}y_{problem_name}": [
                        wandb.Image(
                            mask_to_rgb(img[i], values=[0, 1, 2], colors_cmap=self.colormaps[i])
                        )
    ...
    def validation_step(self, batch: dict, batch_idx):
        ...
        for i in range(len(self.label_names)):
            cm_batch = metrics.compute_confusions(
                y[:, i], pred_categorical[:, i], num_class=2, remove_class_zero=True
            )
            ...
            bce = losses.binary_cross_entropy_loss_mask_invalid(
                logits[:, i], y[:, i], pos_weight=None
            )
...
def mask_to_rgb(
    mask: np.ndarray, values: list[int] = [0, 1, 2, 3], colors_cmap: np.ndarray = COLORS_WORLDFLOODS
) -> np.ndarray:
    """
    ...
    """
    assert len(values) == len(colors_cmap), (
        f"Values and colors should have same length {len(values)} {len(colors_cmap)}"
    )
...
def batch_to_unnorm_rgb(
    ...
    bands_index_rgb = [
        bands_read_names.index(b) for b in ["B4", "B3", "B2"] if b in bands_read_names
    ]
    if len(bands_index_rgb) != 3:
        # try swir/nir/red
        bands_index_rgb = [
            bands_read_names.index(b) for b in ["B11", "B8", "B4"] if b in bands_read_names
        ]
        if len(bands_index_rgb) != 3:
            return
    ...
    if unnormalize:
        ...
        model_input_rgb_npy = np.clip(model_input_rgb_npy / max_clip_val, 0.0, 1.0)

    return model_input_rgb_npy


def unnorm_batch(
    x: torch.Tensor, channel_configuration: str = "all", max_clip_val: float = 3000.0
) -> torch.Tensor:
    model_input_npy = x.cpu().numpy()
...
def plot_batch(
    ...
    for xi, ax in zip(x, axs, strict=False):
...
def plot_batch_output_v1(outputv1: torch.Tensor, axs=None, legend=True, show_axis=False):
    ...
    cmap_preds, norm_preds, patches_preds = plot_utils.get_cmap_norm_colors(
        plot_utils.COLORS_WORLDFLOODS, plot_utils.INTERPRETATION_WORLDFLOODS
    )

    for _i, (xi, ax) in enumerate(zip(outputv1, axs, strict=False)):
        ax.imshow(xi, cmap=cmap_preds, norm=norm_preds, interpolation="nearest")
