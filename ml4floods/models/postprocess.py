...
from ml4floods.data.create_gt import BRIGHTNESS_THRESHOLD, get_brightness


def _watershed_processing(
    prob_water_mask, thres_h=0.6, thres_l=0.4, distance=5, conn=2, watershed_line=True
):
    """
    ...
    prob_water_mask = watershed(
        -prob_water_mask, seed_msk, mask=(prob_water_mask > thres_l), watershed_line=watershed_line
    )
...
def get_mask_watertypes(
    mndwi: np.ndarray | torch.Tensor,
    water_mask: np.ndarray | torch.Tensor,
    permanent_water: np.ndarray | torch.Tensor | None = None,
...
def get_pred_mask_v2(
    ...
    if mask_clouds:
        br = (
            get_brightness(inputs, channels_input=channels_input, collection_name=collection_name)
            > th_brightness
        )
        if hasattr(br, "cpu"):
            br = br.cpu()
...
def compute_cloud_coverage(
    floodmap: gpd.GeoDataFrame, area_imaged: Polygon | MultiPolygon | None = None
) -> float:
    """
...
def get_area_missing_or_cloud(
    floodmap: gpd.GeoDataFrame, area_imaged: Polygon | MultiPolygon
    ...
    area_missing = area_imaged.difference(
        unary_union(floodmap[(floodmap["class"] == "area_imaged")].geometry)
    )
    ...
    if area_missing_or_cloud.geom_type == "GeometryCollection":
        area_missing_or_cloud = unary_union(
            [
                gc
                for gc in area_missing_or_cloud.geoms
                if (gc.geom_type == "Polygon") or (gc.geom_type == "MultiPolygon")
            ]
        )
...
def get_area_missing_or_cloud_or_land(
    floodmap: gpd.GeoDataFrame, area_imaged: Polygon | MultiPolygon
) -> Polygon | MultiPolygon:
    """
    Returns a Polygon with the area of the floodmap that hasn't been imaged (i.e. is out of `floodmap[floodmap['class'] == "area_imaged"]`)
    ...
    #                                    grid_size = 1)
    polygons_in_floodmap = union_all(
        geodataframe_polygonsonly_valid(floodmap[floodmap["class"] != "area_imaged"]).geometry,
        grid_size=1,
    )
    ...
    if area_missing_or_cloud_or_land.geom_type == "GeometryCollection":
        area_missing_or_cloud_or_land = unary_union(
            [
                gc
                for gc in area_missing_or_cloud_or_land.geoms
                if (gc.geom_type == "Polygon") or (gc.geom_type == "MultiPolygon")
            ]
        )
...
def get_area_valid(floodmap: gpd.GeoDataFrame) -> Polygon | MultiPolygon:
    """
...
def get_floodmap_post(
    geojsons: list[str], verbose: bool = False, mode: str = "first"
) -> gpd.GeoDataFrame:
    """
    ...
    return mosaic_floodmaps(
        datas, area_imaged, classes_water=["water", "flood_trace"], verbose=verbose, mode=mode
    )


def mosaic_floodmaps(
    datas: list[gpd.GeoDataFrame],
    area_imaged: Polygon | MultiPolygon,
    ...
    for idx, data in enumerate(datas[1:]):
        if area_not_mapped.is_empty or area_not_mapped.geom_type not in [
            "Polygon",
            "MultiPolygon",
            "GeometryCollection",
        ]:
            if verbose:
                print(
                    f"All area is covered in idx {idx + 1}. Area missing empty: {area_not_mapped.is_empty} Geom type: {area_not_mapped.geom_type}"
                )
            break
        ...
        for c in classes_water:
            water_geoms = data[data["class"] == c].geometry.apply(
                lambda g: g.intersection(area_not_mapped)
            )
        ...
        area_missing_or_cloud = get_area_missing_or_cloud(data, area_imaged).intersection(
            area_missing_or_cloud
        )
        ...
        # update area_missing
        if mode == "max":
            area_not_mapped = get_area_missing_or_cloud_or_land(
                best_floodmap, area_imaged=area_imaged
            ).intersection(area_not_mapped)
        else:
            area_not_mapped = area_missing_or_cloud

    # Join adjacent polygons
    best_floodmap = geodataframe_polygonsonly_valid(best_floodmap)
    best_floodmap["geometry"] = best_floodmap["geometry"].apply(
        lambda x: shapely.set_precision(x, grid_size=1)
    )
    ...
    # Add clouds
    if not area_missing_or_cloud.is_empty and (
        area_missing_or_cloud.geom_type in ["Polygon", "MultiPolygon", "GeometryCollection"]
    ):
    ...
    assert (result.geometry.geom_type != "MultiPolygon").all(), (
        "Everything should be flattened! found some MultiPolygon"
    )
    assert (result.geometry.geom_type != "GeometryCollection").all(), (
        "Everything should be flattened! found some GeometryCollection"
    )
    # Remove geometries that are not polyongs and exclude polygons with area >= 400m^2
    result = result[
        (result.geometry.geom_type == "Polygon") & (result.geometry.area >= 20 * 20)
    ].copy()
...
def compute_pre_post_flood_water(
    floodmap_post_data: gpd.GeoDataFrame, best_pre_flood_data: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
    """
    ...
    area_imaged_post = unary_union(
        floodmap_post_data[floodmap_post_data["class"] == "area_imaged"].geometry
    )

    area_missing_pre = get_area_missing_or_cloud(best_pre_flood_data, area_imaged_post)
    pre_flood_water_or_missing_pre = unary_union(
        best_pre_flood_data[best_pre_flood_data["class"] == "water"].geometry
    ).union(area_missing_pre)
    ...
    # Remove geometries that are not polyongs and exclude polygons with area >= 400m^2
    data_post_flood = data_post_flood[
        (data_post_flood.geometry.geom_type == "Polygon")
        & (data_post_flood.geometry.area >= 20 * 20)
    ].copy()
    ...
    best_pre_flood_data_propagate = best_pre_flood_data[
        best_pre_flood_data["class"] != "flood_trace"
    ].copy()
    best_pre_flood_data_propagate.loc[
        best_pre_flood_data_propagate["class"] == "water", "class"
    ] = "water-pre-flood"
    best_pre_flood_data_propagate.loc[
        best_pre_flood_data_propagate["class"] == "cloud", "class"
    ] = "cloud-pre-flood"
    best_pre_flood_data_propagate.loc[
        best_pre_flood_data_propagate["class"] == "area_imaged", "class"
    ] = "area_imaged-pre-flood"

    post_flood_propagate = floodmap_post_data[
        (floodmap_post_data["class"] == "area_imaged") | (floodmap_post_data["class"] == "cloud")
    ]

    result = pd.concat(
        [best_pre_flood_data_propagate, post_flood_propagate, data_post_flood], ignore_index=True
    )
    result = result[~result.geometry.isna() & ~result.geometry.is_empty]
    return result


def geometrycollection_to_multipolygon(x: GeometryCollection) -> MultiPolygon | Polygon:
...
def geodataframe_polygonsonly_valid(df: gpd.GeoDataFrame | gpd.GeoSeries) -> gpd.GeoDataFrame:
    if isinstance(df, gpd.GeoSeries):
        df = df.geometry.apply(
            lambda x: validation.make_valid(geometrycollection_to_multipolygon(x))
        )
    ...
    df = df[
        (~df.geometry.is_empty) & df.geometry.geom_type.isin(["Polygon", "MultiPolygon"])
    ].copy()
...
def spatial_aggregation(floodmaps_paths: list[str], dst_crs: str = "EPSG:4326") -> gpd.GeoDataFrame:
    ...
    for f in tqdm(floodmaps_paths):
        ...
        data = data[
            ~data.geometry.isna() & ~data.geometry.is_empty & (data.geometry.area > 10**2)
        ].copy()
    ...
    print(
        f"\t{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Correctly dissolved. New shape: {data_all.shape}"
    )
    data_all = data_all.explode(ignore_index=True)
    print(
        f"\t{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Correctly exploded. New shape: {data_all.shape}"
    )

    data_all = data_all[~data_all.geometry.isna() & ~data_all.geometry.is_empty]
    print(
        f"\t{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Removed NA or empty polygons. New shape: {data_all.shape}"
    )

    return data_all


def vectorize_jrc_permanent_water_layer(
    permanent_water: np.ndarray, crs: str, transform: rasterio.transform.Affine
    ...
    data_out = []
    for c, class_name in zip([2, 3], ["seasonal", "permanent"], strict=False):
        geoms_polygons = get_water_polygons(permanent_water == c, transform=transform)
        if len(geoms_polygons) > 0:
            data_out.append(
                gpd.GeoDataFrame({"geometry": geoms_polygons, "class": class_name}, crs=crs)
            )
...
def load_vectorized_permanent_water(
    aoi_folder: str, year: str | None = None
) -> gpd.GeoDataFrame | None:
    """
    ...
    if year is None:
        year = "*"
    files_permanent = fs.glob(
        os.path.join(aoi_folder, "PERMANENTWATERJRC", f"{year}.tif").replace("\\", "/")
    )
    if len(files_permanent) == 0:
        ...
        path_permanent_water_vec = os.path.join(
            aoi_folder, "PERMANENTWATERJRC_vec", namefile_permanent + ".geojson"
        ).replace("\\", "/")
        if not fs.exists(path_permanent_water_vec):
            ...
            permanent_water_floodmap = vectorize_jrc_permanent_water_layer(
                permanent_raster, crs=crs, transform=transform
            )
...
def add_permanent_water_to_floodmap(
    jrc_vectorized_map: gpd.GeoDataFrame, floodmap: gpd.GeoDataFrame, water_class: str | None = None
) -> gpd.GeoDataFrame:
    """
    ...
    if water_class is None:
        if "water" in classes:
            ...
            raise AttributeError("water_class not provided and we couldn't guess it")
