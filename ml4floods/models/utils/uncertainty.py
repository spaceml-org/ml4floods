...
from ml4floods.data.worldfloods.configs import BANDS_S2, CHANNELS_CONFIGURATIONS, COLORS_WORLDFLOODS
...
def compute_uncertainties(dataloader, p_pred_fun, d_pred_fun, num_class, config, num_samples=10):
    ...
    for i, batch in tqdm(
        ...
        compute_uncertainties_for_image_pair(
            test_inputs,
...
def compute_uncertainties_for_image_pair(
    inputs, targets, p_pred_fun, d_pred_fun, num_samples, num_class, config, denorm=True
):
    """
...
def s1_to_unnormed_rgb(image, config):
    ...
    bands_read_names = [
        BANDS_S2[i]
        for i in CHANNELS_CONFIGURATIONS[
    ...
    model_input_rgb_npy = (
        image[:, bands_index_rgb].transpose(0, 2, 3, 1) * std[..., -1::-1] + mean[..., -1::-1]
    )
    model_input_rgb_npy = np.clip(model_input_rgb_npy / 3000.0, 0.0, 1.0)
