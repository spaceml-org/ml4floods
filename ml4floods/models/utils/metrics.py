...
def compute_confusions(
    ...
    test_outputs_categorical: torch.Tensor,
    num_class: int,
    remove_class_zero: bool = False,
) -> torch.Tensor:
    """
    ...
    confusions_batch = torch.zeros(
        size=(ground_truth.shape[0], num_class, num_class), dtype=torch.long
    )
...
def plot_metrics(metrics_dict, label_names, thresholds_water=None):
    ...
    plot_df = pd.DataFrame(
        {"Precision": precisions, "Recall": recalls, "FP Rate": fp_rates, "TP Rate": tp_rates}
    )
...
def compute_metrics(
    ...
    pred_fun: Callable,
    thresholds_water=np.arange(0, 1, 0.05),
    threshold: float = 0.5,
    plot=False,
    mask_clouds: bool = False,
    convert_targets: bool = True,
) -> dict:
    """
    ...
    for i, batch in tqdm(
        enumerate(dataloader), total=int(len(dataloader.dataset) / dataloader.batch_size)
    ):
        ...
        if mask_clouds:
            ...
            assert test_outputs.shape[1] == num_class, (
                f"Mode normal expects {num_class} channel output image found {test_outputs.shape}"
            )
        ...
        if ground_truth.shape[1] > 1:
            ...
            invalids = (water_ground_truth == 0).to(
                test_outputs_categorical.device
            )  # (batch_size, H, W)
            ground_truth_outputs = torch.clone(water_ground_truth).to(
                test_outputs_categorical.device
            )
        else:
            # Version v1
            ground_truth_outputs = torch.clone(
                ground_truth[:, 0].to(test_outputs_categorical.device)
            )
            # Save invalids to discount
        ...
        confusions_batch = compute_confusions(
            ground_truth_outputs,
            test_outputs_categorical,
            num_class=num_class,
        ...
        # Discount invalids
        inv_substract = torch.sum(invalids, dim=(1, 2)).to(
            confusions_batch.device
        )  # (batch_size, )
        ...
        # Set clouds to land
        test_outputs_categorical_thresh = torch.zeros(
            ground_truth_outputs.shape, dtype=torch.long, device=ground_truth_outputs.device
        )
        ...
        for thr in thresholds_water:
            ...
            confusions_batch = compute_confusions(
                ground_truth_outputs,
                test_outputs_categorical_thresh,
                num_class=2,
                remove_class_zero=False,
            )  # [batch_size, 2, 2]
...
def group_confusion(
    confusions: torch.Tensor, cems_code: np.ndarray, fun: Callable, label_names: list[str]
) -> list[dict[str, Any]]:
    CMs = OrderedDict({c: [] for c in sorted(np.unique(cems_code))})

    for code, cms in zip(cems_code, confusions, strict=False):
        CMs[code].append(cms)
...
def compute_metrics_v2(
    ...
    if not thresholds_water:
        thresholds_water = (
            [0, 1e-3, 1e-2] + np.arange(0.05, 0.96, 0.05).tolist() + [0.99, 0.995, 0.999]
        )
    ...
    for i, batch in tqdm(
        ...
        if not mask_clouds:
            assert test_outputs.shape[1] == 2, (
                f"Mode mask clouds expects 2 channel output image found {test_outputs.shape}"
            )
...
def compute_positives(
    ground_truth_outputs: torch.Tensor,
    water_outputs_categorical: torch.Tensor,
    convert_targets: bool = True,
) -> torch.Tensor:
    """
