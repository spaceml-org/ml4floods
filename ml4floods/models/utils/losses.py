...
def dice_loss_mask_invalid(logits: torch.Tensor, target: torch.Tensor, smooth=1.0) -> float:
    """
    ...
    """
    assert logits.dim() == 4, "Expected 4D tensor logits"
    assert target.dim() == 3, "Expected 3D tensor target"

    pred = torch.softmax(logits, dim=1)
    valid = target != 0  # (B, H, W) tensor
    target_without_invalids = (target - 1) * valid  # Set invalids to land

    target_one_hot_without_invalid = torch.nn.functional.one_hot(
        target_without_invalids, num_classes=pred.shape[1]
    ).permute(0, 3, 1, 2)
    axis_red = (2, 3)  # H, W reduction

    pred_valid = (
        pred * valid.unsqueeze(1).float()
    )  # # Set invalids to 0 (all values in prob tensor are 0

    intersection = (pred_valid * target_one_hot_without_invalid).sum(dim=axis_red)  # (B, C) tensor

    union = pred_valid.sum(dim=axis_red) + target_one_hot_without_invalid.sum(
        dim=axis_red
    )  # (B, C) tensor
...
def cross_entropy_loss_mask_invalid(
    ...
    bce = F.cross_entropy(
        logits, target_without_invalids, weight=weight, reduction="none"
    )  # (B, 1, H, W)
...
def binary_cross_entropy_loss_mask_invalid(
    logits: torch.Tensor, target: torch.Tensor, pos_weight: Optional = None
) -> float:
    """
    ...
    """
    assert logits.dim() == 3, (
        f"Unexpected shape of logits. Logits: {logits.shape} target: {target.shape}"
    )
    assert target.dim() == 3, (
        f"Unexpected shape of target. Logits: {logits.shape} target: {target.shape}"
    )

    valid = target != 0
    target_without_invalids = (target - 1) * valid

    pixelwise_bce = F.binary_cross_entropy_with_logits(
        logits, target_without_invalids.float(), reduction="none", pos_weight=pos_weight
    )
...
def calc_loss_multioutput_logistic_mask_invalid(
    logits: torch.Tensor,
    target: torch.Tensor,
    pos_weight_problem: list[float] | None = None,
    weight_problem: list[float] | None = None,
) -> float:
    ...
    for i in range(logits.shape[1]):
        ...
        curr_loss = binary_cross_entropy_loss_mask_invalid(
            logits[:, i], target[:, i], pos_weight=pos_weight
        )
