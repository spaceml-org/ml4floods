...
def _get_GEE_ids(session, start_date, end_date, aoi_wgs):
    ...
    try:
        ...
        cloud_coverages = [
            image["properties"]["CLOUD_COVERAGE_ASSESSMENT"]
            for image in json.loads(content)["images"]
        ]
...
class RESTMosaic:
    def __init__(
        ...
        if s2_tiles_path:
            ...
            self.s2_tiles = gpd.read_file(
                os.path.join(os.getcwd(), "assets", "S2_tiles.gpkg")
            ).set_index("Name")
        else:
            if self.verbose:
    ...
    def mosaic(self, aoi_wgs, start_date, end_date=None, cloud_dest=None):
        ...
        (
            ...
            self.reverse_proj,
            self.UTM_EPSG,
        ) = self._get_proj_and_affine(aoi_wgs)
        ...
        _ids, cloud_coverages, geometries = _get_GEE_ids(
            self.session, start_date, end_date, geometry.box(*aoi_wgs.exterior.bounds)
        )
        self.image_geometries = dict(zip(_ids, geometries, strict=False))
        _ids = [x for _, x in sorted(zip(cloud_coverages, _ids, strict=False))]
        ...
        if False:
            ...
            if self.verbose:
                self.logger.info("calling mosaic. Filling Alpha mask.")
            ...
            for ii, _id in enumerate(self.image_ids):
                run_idx[_id] = list(
                    set(
                        list(
                            zip(
                                self.px_x[self.alpha_mask[:, :, ii] > 0].tolist(),
                                self.px_y[self.alpha_mask[:, :, ii] > 0].tolist(),
                                strict=False,
                            )
                        )
                    )
            ...
            if self.verbose:
                self.logger.info("Filling mosaic")
    ...
    def _mask_by_id(self):
        ...
        for ii, _id in enumerate(self.image_ids):
            if type(self.s2_tiles) == gpd.GeoDataFrame:
                ...
                granule = granule.intersection(
                    geometry.shape(self.image_geometries[_id])
                )  # get intersection with image pixels
                granule_utm = ops.transform(
                    self.reproject, granule
                )  # flatten and transform the granule to utm
                granule_utm = ops.transform(
                    lambda x, y, z=None: (x, y), granule_utm
                )  # remove the z coordinates
            else:
                granule_ft = utils.load_json_from_bucket(
                    "ml4floods", f"worldfloods/s2_tiles/{_id[-5:]}.json"
                )
            ...
            granule_px = affine_transform(
                granule_utm, self.GT
            )  # get the granule shape into the pixel coordinates
    ...
    def _fill_single_id(self, _id):
        ...
        for ii_x in range(self.Dx // self.PATCH_SIZE + 1):
            for ii_y in range(self.Dy // self.PATCH_SIZE + 1):
                ...
                im_arr[
                    ii_y * self.PATCH_SIZE : (ii_y + 1) * self.PATCH_SIZE,
                    ii_x * self.PATCH_SIZE : (ii_x + 1) * self.PATCH_SIZE,
                    :,
                ] = arr[
                    : im_arr.shape[0] - ii_y * self.PATCH_SIZE,
                    : im_arr.shape[1] - ii_x * self.PATCH_SIZE,
                    :,
                ]
    ...
    def _make_alpha_mask(self, Dx, Dy):
        ...
        alpha_mask = np.zeros(
            (Dy, Dx, len(self.image_ids))
        )  # the main alpha mask to determine api calls
        for ii, _id in enumerate(self.image_ids):
            if ii == 0:  # first mask do nothing
                ...
                alpha_mask[..., ii] = (
                    self.mask_arr[..., ii] - self.mask_arr[..., :ii].sum(axis=-1)
                ).clip(0, 1)  # do these pixels for this image
    ...
    def _fill_mosaic(self):
        ...
        if self.workers == 1:
            ...
            if self.verbose:
                self.logger.info(
                    f"Calling REST API for {len(args)} patches with {self.workers} workers"
                )
            ...
            chunk_args = [
                (args[ii_w * CHUNK : (ii_w + 1) * CHUNK], self.google_key_path, loggers[ii_w])
    ...
    def _write_rio(self):
        with rasterio.open(
            ...
            transform=rasterio.transform.from_origin(
                west=self.aoi_utm.bounds[0],
                north=self.aoi_utm.bounds[3],
                xsize=self.SCALE,
                ysize=self.SCALE,
            ),
        ) as dst:
            dst.write(
                np.transpose(self.im_arr, [2, 0, 1]), indexes=range(1, len(self.BANDS) + 1)
            )  # rast io goes channels first
...
if __name__ == "__main__":
    ...
    aoi_wgs = geometry.Polygon(
        [
            [-74.53043581870358, 46.03607242419274],
