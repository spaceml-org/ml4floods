...
countries_df = pd.DataFrame(
    json.load(
        open(
            os.path.join(
                os.getcwd(), "ml4floods", "serve", "tileserver", "static", "countries.json"
            )
        )
    )
)
...
def get_geojson():
    ...
    return send_file(
        os.path.join(os.getcwd(), "tmp", "gj_gdf.geojson"),
        attachment_filename=f"geojson_{dt.now().isoformat()}.json",
    )
...
def get_runingest():
    ...
    if "CUSTOM" in ems_code:
        ...
        ingestor.ingest(
            ems_code=ems_code, aoi_code=aoi_code, code_date=event_date, aoi_geom=aoi_geom
        )
        ...
        ingestor.ingest(ems_code=ems_code, aoi_code=aoi_code, code_date=event_date, aoi_geom=None)
...
def get_isavailable():
    ...
    fname = os.path.join(
        "worldfloods", "lk-dev", "S2-post", "_".join([ems_code, AOI, "S2.tif"])
    )  # <- need to check this
    ...
    if "CUSTOM" in ems_code:
        return jsonify(
            utils.load_json_from_bucket(
                "ml4floods",
                os.path.join("worldfloods", "lk-dev", "meta", f"{ems_code}_{AOI}_custom.json"),
            )
        )
    else:
        return jsonify(
            utils.load_json_from_bucket(
                "ml4floods",
                os.path.join("worldfloods", "lk-dev", "ems-aoi", f"{ems_code}_{AOI}.json"),
            )
        )
...
def refresh_catalog():
    ...
    if iso2:
        idxs = (
            (gj_gdf["event-date"] >= start_date_object)
            & (gj_gdf["event-date"] < end_date_object)
            & gj_gdf["value"].str.contains(iso2)
        )  # change to np.prod.any in list for multi
    else:
        idxs = (gj_gdf["event-date"] >= start_date_object) & (
            gj_gdf["event-date"] < end_date_object
        )
...
def servexyz(layer_name, image_name, z, x, y):
    ...
    if layer_name == "null":
        return send_file(
            os.path.join(os.getcwd(), "ml4floods", "serve", "tileserver", "static", "border.png")
        )
    ...
    try:
        ...
        if "S2" in layer_name:
            ...
            print(
                ...
                utils.check_file_in_bucket_exists(
                    "ml4floods", f"worldfloods/lk-dev/{layer_name}/{image_name}_S2.tif"
                ),
            )
            ...
            print(
                "CHECK",
                utils.check_file_in_bucket_exists(
                    "ml4floods", f"worldfloods/lk-dev/gt/{image_name}_GT.tif"
                ),
            )
            ...
            READBANDS = [1]
            image_address = (
                f"gs://ml4floods/worldfloods/lk-dev/ML/{image_name}_ML_{chosen_model}.tif"
            )
            print(
                "CHECK",
                utils.check_file_in_bucket_exists(
                    "ml4floods", f"worldfloods/lk-dev/ML/{image_name}_ML_{chosen_model}.tif"
                ),
            )
        else:
            raise
        ...
        with rasterio.Env(GDAL_DISABLE_READDIR_ON_OPEN=True, CPL_DEBUG=True):
            with rasterio.open(image_address, "r") as rst:
                ...
                if (
                    ...
                    or ((window.col_off - rst.shape[0]) >= window.width)
                    or (window.row_off <= -window.height)
                    or ((window.row_off - rst.shape[1]) >= window.height)
                ):
                    print("out of range")
                    return send_file(
                        os.path.join(
                            os.getcwd(), "ml4floods", "serve", "tileserver", "static", "border.png"
                        )
                    )

                # use rasterio to read the pixel window from the cloud bucket
                rst_arr = rst.read(
                    READBANDS, window=window, out_shape=OUTPUT_SHAPE, boundless=True, fill_value=0
                )
        ...
        if "S2" in layer_name:
            ...
            col_dict = {
                ...
                2: [0, 0, 255],
                3: [255, 0, 0],
            }  # 0: nodata; 1: land; 2: water; 3: cloud
