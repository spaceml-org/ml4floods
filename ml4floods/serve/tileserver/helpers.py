...
def postprocess_floodtable(events_df, countries_df):
    ...
    events_df = pd.merge(
        events_df, countries_df, how="left", left_on="country_lower", right_on="label_lower"
    )
    events_df.loc[events_df["value"].isna(), "value"] = events_df.loc[
        events_df["value"].isna(), "country_lower"
    ].map(COUNTRIES_MAP)
    return events_df
                    ...
                    "geometry": geometry.mapping(
                        geometry.Polygon(
                            [
                                ...
                                for el in list(zip(t[1::2], t[::2], strict=False))
                            ]
                        )
                    ),  # lon, lat
                }
...
def _ingest_event_aoi(ems_code):
    ...
    for record in records:
        ft = geojson.Feature(
            geometry=record["geometry"], properties={"CODE": record["CODE"], "AOI": record["AOI"]}
        )
        tmp_path = os.path.join(os.getcwd(), "tmp", f"{ems_code}_{record['AOI']}.json")
        cloud_path = os.path.join(
            "gs://ml4floods", "worldfloods", "lk-dev", "ems-aoi", f"{ems_code}_{record['AOI']}.json"
        )
...
def walk_bucket(events_df):
    ...
    ems_blobs = [
        ...
        for blob in client.list_blobs(
            "ml4floods", prefix=os.path.join("worldfloods", "lk-dev", "ems-aoi")
        )
        if blob.name[-5:] == ".json"
    ]  # .json -> filter root
    ...
    custom_blobs = [
        blob.name
        for blob in client.list_blobs(
            "ml4floods", prefix=os.path.join("worldfloods", "lk-dev", "meta")
        )
        if "CUSTOM" in blob.name and blob.name[-5:] == ".json"
    ]

    print("walk bucket -> custom blobs:", len(custom_blobs))
    custom_events = []
    for b in custom_blobs:
        blob_json = utils.load_json_from_bucket(
            "ml4floods", os.path.join("worldfloods", "lk-dev", "meta", os.path.split(b)[1])
        )
...
def refresh_geojson(gj_gdf, updated_df):
    ...
    missing_custom_events = updated_df.loc[
        updated_df["Code"].str.contains("CUSTOM") & ~updated_df["Code"].isin(gj_gdf["CODE"]), "Code"
    ].values.tolist()
    ...
    missing_blobs = [
        ...
        if os.path.split(b.name)[1].split("_")[0] in missing_custom_events
    ]
    missing_records = []
        ...
        missing_records.append(
            {
                ...
                "TITLE": os.path.split(b)[1].split("_")[0]
                + "_"
                + blob_json["properties"]["event-date"],
    ...
    custom_df = pd.DataFrame(
        missing_records, columns=["CODE", "AOI", "TITLE", "event-date", "geometry", "value"]
    )
    ...
    new_blobs = [
        b
        for b in ems_blobs
        if os.path.split(b)[1].split("_")[0] + "_" + os.path.split(b)[1].split("_")[1][:-5]
        not in gj_gdf["idx"].values.tolist()
    ]
    ...
    records = [
        {
            ...
            "geometry": geometry.shape(
                utils.load_json_from_bucket(
                    "ml4floods",
                    os.path.join("worldfloods", "lk-dev", "ems-aoi", os.path.split(b)[1]),
                )["geometry"]
            ),
        }
        for b in new_blobs
    ...
    new_df = (
        pd.merge(
            new_df,
            updated_df[["Code", "CodeDate", "Title"]],
            how="left",
            left_on="CODE",
            right_on="Code",
        )
        .rename(columns={"CodeDate": "event-date", "Title": "TITLE"})
        .drop(columns=["Code"])
    )
    ...
    gj_gdf = pd.merge(
        gj_gdf, updated_df[["Code", "value"]], how="left", left_on="CODE", right_on="Code"
    ).drop(columns=["Code"])
