...
class Ingestor:
    def __init__(
        ...
        ML_models=["linear", "unet", "simplecnn"],
    ):
        """
    ...
    def ingest(self, ems_code, code_date, aoi_code, aoi_geom=None):
        ...
        if "ML" in self.include and self.inference_endpoint is not None:
            ...
            assert json.loads(r.text)["status"] == "success", (
                "Inference server unavailable - check server is active?"
            )
            ...
            for chosen_model in self.ML_models:
                ...
                src_path = os.path.join(
                    "gs://ml4floods",
                    self.cloud_path["S2-post"],
                    f"{self.ems_code}_{self.aoi_code}_S2.tif",
                )
                dest_path = os.path.join(
                    "gs://ml4floods",
                    self.cloud_path["ML"],
                    f"{self.ems_code}_{self.aoi_code}_ML_{chosen_model}.tif",
                )
    ...
    def _ingest_vector(self):
        ...
        zip_df["pref_monprod"] = (
            zip_df["monprod"].map({"PROD": 0, "MONI": 1}).fillna(2)
        )  # prefer PRODUCT to MONITOR0X to NaN
        zip_df["pref_vecrtp"] = (
            zip_df["vecrtp"].map({"VEC": 0, "RTP": 1}).fillna(2)
        )  # prefer VECTOR to RTP (ready-to-print) to NaN
        ...
        for idx, row in zip_df.iterrows():
            ...
            metadata_floodmap = activations.filter_register_copernicusems(
                unzipped_file, self.code_date
            )
            self.logger.info("retreived metadata_floodmap")
            ...
            if metadata_floodmap is not None:
                ...
                self.register = {
                    "id": "_".join([self.ems_code, self.aoi_code]),
    ...
    def _ingest_S2_register(self):
        ...
        if self.register is not None:
            ...
            aoi = geometry.shape(
                utils.load_json_from_bucket(
                    "ml4floods",
                    os.path.join(
                        "worldfloods", "lk-dev", "ems-aoi", f"{self.ems_code}_{self.aoi_code}.json"
                    ),
                )["geometry"]
            )
        ...
        if "S2-pre" in self.include:
            save_dest = os.path.join(
                self.cloud_path["S2-pre"], f"{self.ems_code}_{self.aoi_code}_S2"
            )
            end_date = self.date_obj - timedelta(days=1)
            start_date = end_date - timedelta(days=21)
        ...
        if "S2-post" in self.include:
            save_dest = os.path.join(
                self.cloud_path["S2-post"], f"{self.ems_code}_{self.aoi_code}_S2"
            )
            start_date = self.date_obj
    ...
    def _ingest_S2_EE(self, _id, aoi, start_date, end_date, save_dest):
        ...
        img_export = (
            img_export.select(self.BANDS_EXPORT).toFloat().clip(ee_boundary)
    ...
    def _construct_GT(self):
        ...
        self.logger.info(
            "CHECK BUCKET: "
            + str(
                utils.check_file_in_bucket_exists(
                    "ml4floods",
                    os.path.join(
                        self.cloud_path["S2-post"], f"{self.ems_code}_{self.aoi_code}_S2.tif"
                    ),
                )
            )
        )
        ...
        self.register["floodmap"].to_file(
            os.path.join(self.local_path, self.register["id"] + "_vector.geojson"), driver="GeoJSON"
        )

        ### prep meta for json
        self.register["metadata_floodmap"]["area_of_interest_polygon"] = geometry.mapping(
            self.register["metadata_floodmap"]["area_of_interest_polygon"]
        )
        self.register["metadata_floodmap"]["satellite date"] = self.register["metadata_floodmap"][
            "satellite date"
        ].isoformat()
        self.register["metadata_floodmap"]["timestamp_pre_event"] = self.register[
            "metadata_floodmap"
        ]["timestamp_pre_event"].isoformat()
        ...
        # move all to cloud
        to_cloud = {
            os.path.join(self.local_path, self.register["id"] + "_vector.geojson"): os.path.join(
                "gs://" + self.bucket,
                self.cloud_path["meta"],
                self.register["id"] + "_vector.geojson",
            ),
            os.path.join(self.local_path, self.register["id"] + "_aoi.json"): os.path.join(
                "gs://" + self.bucket, self.cloud_path["meta"], self.register["id"] + "_aoi.json"
            ),
            os.path.join(self.local_path, self.register["id"] + "_meta.json"): os.path.join(
                "gs://" + self.bucket, self.cloud_path["meta"], self.register["id"] + "_meta.json"
            ),
            os.path.join(self.local_path, self.register["id"] + "_GT.tif"): os.path.join(
                "gs://" + self.bucket, self.cloud_path["GT"], self.register["id"] + "_GT.tif"
            ),
        }
...
if __name__ == "__main__":
    ...
    ingestor.ingest(
        ems_code="EMSR411",  #'EMSR267',#EMSR255_05MARIENBERG  EMSR255  EMSR411 AOI01
