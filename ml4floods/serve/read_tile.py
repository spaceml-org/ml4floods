...
def round_outer_window(window: rasterio.windows.Window) -> rasterio.windows.Window:
    ...
    return window.round_lengths(op="ceil", pixel_precision=PIXEL_PRECISION).round_offsets(
...
def read_tile(
    ...
    x: int,
    y: int,
    size_output: tuple[int, int] = (SIZE, SIZE),
    ...
    with (
        rasterio.Env(GDAL_DISABLE_READDIR_ON_OPEN=True, CPL_DEBUG=True),
        rasterio.open(tiff_path, "r") as rst,
    ):
        x1, y1, x2, y2 = rst.bounds
        rst_bounds = min(x1, x2), min(y1, y2), max(x1, x2), max(y1, y2)
        if rst.crs != WEB_MERCATOR_CRS:
            bounds_read_crs_dest = warp.transform_bounds(
                {"init": WEB_MERCATOR_CRS},
                rst.crs,
                left=bounds_wgs.left,
                top=bounds_wgs.top,
                bottom=bounds_wgs.bottom,
                right=bounds_wgs.right,
            )
        else:
            bounds_read_crs_dest = (
                bounds_wgs.left,
                bounds_wgs.bottom,
                bounds_wgs.right,
                bounds_wgs.top,
        ...
        if rst.crs != WEB_MERCATOR_CRS:
            window = pad_window(
                window, (1, 1)
            )  # Add padding for bicubic int or for co-registration
    ...
    input_output_factor = (
        window.height / output_shape_read[1],
        window.width / output_shape_read[2],
    )

    transform_window = rasterio.windows.transform(window, rst_transform)
    transform_src = rasterio.Affine(
        transform_window.a * input_output_factor[1],
        transform_window.b,
        transform_window.c,
        transform_window.d,
        transform_window.e * input_output_factor[0],
        transform_window.f,
    )

    transform_dst = rasterio.transform.from_bounds(
        west=bounds_wgs.left,
        north=bounds_wgs.top,
        east=bounds_wgs.right,
        south=bounds_wgs.bottom,
        width=size_output[0],
    ...
    rst_arr, rst_transform = warp.reproject(
        source=src_arr,
        destination=rst_arr,
