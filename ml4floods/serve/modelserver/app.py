...
for model_name, conf in models_conf.items():
    opt = get_default_config(
        f"gs://ml4cc_data_lake/0_DEV/2_Mart/2_MLModelMart/{conf['experiment_name']}/config.json"
    )
    ...
    N_CHANNELS = len(
        CHANNELS_CONFIGURATIONS[opt["model_params"]["hyperparameters"]["channel_configuration"]]
    )
    ...
    logger.info(
        ...
        + str(
            utils.check_file_in_bucket_exists(
                "ml4floods",
                os.path.join("worldfloods", "lk-dev", "S2-post", f"{CODE}_{AOI}_S2.tif"),
            )
        )
    )
        ...
        with rasterio.open(tif_source, "r") as rst:
            ...
            for ii_x in range(rst.shape[1] // PATCH_SIZE + 1):
                for ii_y in range(rst.shape[0] // PATCH_SIZE + 1):
                    w = rst.read(
                        list(range(1, 14)),
                        window=Window(ii_x * PATCH_SIZE, ii_y * PATCH_SIZE, 512, 512),
                    )
                    ...
                    Y_arr[
                        ii_y * PATCH_SIZE : min((ii_y + 1) * PATCH_SIZE, rst.shape[0]),
                        ii_x * PATCH_SIZE : min((ii_x + 1) * PATCH_SIZE, rst.shape[1]),
                    ] = Y_h[
                        : min(PATCH_SIZE, rst.shape[0] - ii_y * PATCH_SIZE),
                        : min(PATCH_SIZE, rst.shape[1] - ii_x * PATCH_SIZE),
                    ]
